...
export[...] {
  eventdef['cel:onmousedown'] {
    description = [[Signaled when a mouse button is pressed.]];

    synopsis = [==[
      cel:onmousedown(state, button, x, y, intercepted)
    ]==];

    params = {
      param.cel[[self - self]];
      param.number[[state - 3 means a tripleclick, 2 a doubleclick, 1 is a normal click.]];
      param.mousebutton[[button - The mouse button that was pressed down.  A valid entry in mouse.buttons.]];
    ...
    returns = {
      param.boolean[[intercepted - Return true to intercept the event.  This will indicate to any host cels that
        see the event has been intercepted, generally meaning some action was taken.]];
    };
  };
